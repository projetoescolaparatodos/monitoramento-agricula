...
function Gestor() {
    ...
    const fetchSectorInfoAndChartData = async () => {
      ...
      for (const sector of sectors) {
        ...
        if (sectorSnap.exists()) {
          info[sector] = { ...sectorSnap.data(), chartData: sectorSnap.data().chartData || [] };
        }
      }
      setSectorInfo(info);
  ...
  const handleChartDataChange = (index, field, value) => {
    setChartDataState(prev => prev.map((item, i) =>
      i === index ? { ...item, [field]: field === 'value' ? Number(value) : value } : item
    ));
  };
  ...
  const handleSave = async () => {
    try {
      ...
      await updateDoc(doc(db, "setores", selectedSector), { chartData: chartDataState }); //Update chartData in setores collection
      toast({
        title: "Sucesso",
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Textarea
                      ...
                      onChange={(e) => handleChange(selectedSector, "description", e.target.value)}
                      rows={4}
                    />
                    <Textarea
                      className="mb-2"
                      ...
                      onChange={(e) => handleChange(selectedSector, "goals", e.target.value)}
                      rows={4}
                    />
                    <Textarea
                      placeholder="Realizações"
                      value={sectorInfo[selectedSector].achievements}
                      onChange={(e) => handleChange(selectedSector, "achievements", e.target.value)}
                      rows={4}
                    />
                  </div>
                  <div>
                    <h3 className="text-lg font-medium mb-2">Upload de Mídia</h3>
                    <Upload folder={`setores/${selectedSector}/media`} onUploadComplete={(urls) => {
                      handleChange(selectedSector, "mediaUrls", [...(sectorInfo[selectedSector].mediaUrls || []), ...urls]);
                    }} />
                  </div>
                </div>
