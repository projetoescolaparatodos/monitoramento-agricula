        ...
        <main className="container mx-auto px-2 sm:px-4 pt-28 pb-16 relative z-10 overflow-x-hidden">
          ...
          <div className="space-y-12">
            <div className="prose max-w-none">
              <h1 className="section-title text-3xl sm:text-4xl lg:text-5xl font-bold text-center mb-4 text-white">PESCA</h1>
            ...
            <section className="mt-12 sm:mt-16">
              <div className="p-4 sm:p-6">
                ...
                <div className="flex flex-col lg:flex-row justify-center gap-4 mt-6 sm:mt-8">
                  <FishingTabButton className="shadow-md text-sm sm:text-base lg:text-lg py-3 px-4 sm:px-6 rounded-lg">
                    Iniciar Cadastro na Psicultura
                  </FishingTabButton>

                  <Button
                    onClick={() => setLocation("/pesca/map")}
                    className="flex items-center gap-2 bg-primary hover:bg-primary/90 text-sm sm:text-base lg:text-lg py-3 px-4 sm:px-6 rounded-lg shadow-md"
                  >
                    <Map className="h-4 w-4 sm:h-5 sm:w-5 lg:h-6 lg:w-6 flex-shrink-0" />
                    <span className="text-sm sm:text-base lg:text-lg">Acompanhe nossas Atividades em tempo real</span>
                  </Button>
                </div>
              </div>
            </section>
            ...
            <section className="mt-12 sm:mt-16 rounded-lg p-2 sm:p-6 lg:p-8">
              <h2 className="section-title text-2xl sm:text-3xl lg:text-4xl font-bold text-center mb-6 sm:mb-8 text-white">ATIVIDADES DA PESCA</h2>
              {pescaData ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8 w-full">
                  <Card className="w-full">
                    <CardHeader className="pb-2">
                      <CardTitle className="flex items-center gap-2 text-sm sm:text-base lg:text-lg">
                        <Fish className="h-4 w-4 sm:h-5 sm:w-5 lg:h-6 lg:w-6 text-blue-500 flex-shrink-0" />
                        <span className="truncate">Produção Total de Pescado</span>
                      </CardTitle>
                    </CardHeader>
                    <CardContent className="pt-0">
                      <p className="text-2xl sm:text-3xl lg:text-4xl font-bold">{(pescaData.reduce((sum, p) => sum + (p.quantidadePescado || 0) + (p.quantidadeAlevinos || 0), 0) || 0).toFixed(2)} kg</p>
                    </CardContent>
                  </Card>
                  <Card className="w-full">
                    <CardHeader className="pb-2">
                    ...
                    <CardContent className="pt-0">
                      <p className="text-2xl sm:text-3xl lg:text-4xl font-bold">{pescaData.length || 0}</p>
                    </CardContent>
                  </Card>
                  ...
                  <CardContent className="p-0">
                    <div className="overflow-x-auto">
                      <Table className="w-full min-w-[800px] [&_th]:text-black [&_td]:text-gray-700 [&_tr:hover]:bg-gray-50/50">
                        <TableHeader>
                          <TableRow>
                            <TableHead className="text-sm lg:text-base">Localidade</TableHead>
                            <TableHead className="text-sm lg:text-base">Proprietário</TableHead>
                            <TableHead className="text-sm lg:text-base">Espécie de Peixe</TableHead>
                            <TableHead className="text-sm lg:text-base">Tipo de Sistema</TableHead>
                            ...
                            <TableHead className="text-sm lg:text-base">Quantidade KG</TableHead>
                        ...
                        <TableBody>
                          {pescaData?.slice(0, visibleItems).map((pesca) => (
                            <TableRow key={pesca.id}>
                              <TableCell className="text-sm lg:text-base">{pesca.localidade || '-'}</TableCell>
                              <TableCell className="text-sm lg:text-base">{pesca.proprietario || '-'}</TableCell>
                              <TableCell className="text-sm lg:text-base">{pesca.especiePeixe || '-'}</TableCell>
                              <TableCell className="text-sm lg:text-base">{pesca.estrutura || pesca.tipoTanque || '-'}</TableCell>
                              <TableCell className="text-sm lg:text-base">{pesca.tecnicoResponsavel || '-'}</TableCell>
                              <TableCell className="text-sm lg:text-base">{new Date(pesca.dataCadastro).toLocaleDateString()}</TableCell>
                              <TableCell>
                                <span className={`text-sm lg:text-base font-medium ${pesca.concluido ? 'text-green-600' : 'text-blue-600'}`}>
                                  {pesca.concluido ? 'Concluído' : 'Em Andamento'}
                                </span>
                              </TableCell>
                              <TableCell className="text-sm lg:text-base">{pesca.quantidadeAlevinos ? pesca.quantidadeAlevinos.toFixed(2) : '0.00'}</TableCell>
                              <TableCell className="text-sm lg:text-base">{pesca.areaAlagada ? pesca.areaAlagada.toFixed(2) : '0.00'}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      </Table>
