...
const FormPescaCompleto = () => {
  ...
  useEffect(() => {
    ...
    const savedDraft = localStorage.getItem('formPescaCompletoDraft');
    if (savedDraft) {
      try {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      localStorage.setItem('formPescaCompletoDraft', JSON.stringify(formData));
    }, 2000);
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-50 p-4 relative">
      ...
      <Card className="w-full max-w-4xl">
        <CardHeader className="bg-blue-800 text-white">
          <CardTitle>Cadastro Completo - Setor de Pesca</CardTitle>
...
export default FormPescaCompleto;
